...
import Arrow from '../../../element/icons/arrow';
...
const Carousel = ({ children, style, className }) => {
  ...
  const scrollCarouselToElement = (foundElements, direction) => {
    const liIndex = foundElements.findIndex((element) =>
      element.classList.contains('respark_crsl-item'),
    );
    if (liIndex !== -1) {
  ...
  return (
    ...
    >
      {isPrevArrowShown && (
        <StyledPrevArrowWrapper
          className="respark_crsl-prev-button"
      ...
      <StyledCarousel className={`${className}-items`} ref={carouselRef}>
        {childrenArray.map((child) => (
          <StyledCarouselItem
            className={`${className}-item-wrapper respark_crsl-item`}
            key={child.key}
          >
            {child}
          </StyledCarouselItem>
      ...
      {isNextArrowShown && (
        <StyledNextArrowWrapper
          className="respark_crsl-next-button"
