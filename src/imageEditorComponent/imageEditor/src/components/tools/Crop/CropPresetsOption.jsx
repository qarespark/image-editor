...
const CropPresetsOption = ({ anchorEl, onClose }) => {
  ...
  return (
    <>
      <StyledToolsBarItemButtonLabel className="respark_crop-tool-label respark_selected-crop-preset-label">
        {t(toolTitleKey)}
      </StyledToolsBarItemButtonLabel>
      <StyledOpenMenuButton
        className="respark_crop-presets-opener-button"
        color="link"
        size="lg"
      >
      ...
      <Menu
        className="respark_crop-presets-menu"
        anchorEl={anchorEl}
        enableOverlay
        onClose={onClose}
