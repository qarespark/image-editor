...
import MenuItem from '../../../../element/ui/core/menu-item';
import FontBold from '../../../../element/icons/font-bold';
import FontItalic from '../../../../element/icons/font-italic';
...
const TextControls = ({ text, saveText, children }) => {
  ...
  const changeTextProps = useCallback(
    (e) => {
      ...
      saveText((latestText) => ({
        id: latestText.id,
        [name]: type === 'number' ? restrictNumber(value, 1, 150) : value,
      }));
    },
    [saveText],
  );
  ...
  return (
    <AnnotationOptions
      className="respark_text-tool-options"
      annotation={text}
    ...
    >
      {Array.isArray(fonts) && fonts.length > 1 && (
        <StyledFontFamilySelect
          className="respark_text-font-family-option"
      ...
      <StyledFontSizeInput
        className="respark_text-size-option"
        value={text.fontSize || ''}
        name="fontSize"
      ...
      {!useCloudimage && (
        <>
          <StyledIconWrapper
            className="respark_text-bold-option text-style-icon"
          ...
          <StyledIconWrapper
            className="respark_text-italic-option text-style-icon"
