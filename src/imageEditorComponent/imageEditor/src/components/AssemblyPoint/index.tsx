...
const AssemblyPoint = (props) => {
  ...
  const [editorConfig, setEditorConfig] = useState<any>(initialConfig);
  ...
  const [parentConfigMesg, setParentConfigMesg] = useState(initialConfig);
  const { currentId, currentAction, setCurrentAction, setCurrentId } = useContext(EditorContext);
  ...
  const getEmptyAppPropsObj = () => {
    return {
      source: initialBg,
      onSave: (editedImageObject, designState) => { },    // setCurrentAction({type:'',action:''});,
      ...
      defaultTabId: TABS.TEMPLATE,
      defaultToolId: TOOLS.TEXT
    }
  }
  const [appProps, setAppProps] = useState({ ...getEmptyAppPropsObj() });
  const [defaultAndProvidedConfig, setDefaultAndProvidedConfig] = useState(deepMerge(defaultConfig, appProps));

  useEffect(() => {
    setDefaultAndProvidedConfig(appProps)
  }, [appProps])
  ...
  useEffect(() => {
    if (activeTemplate?.designState?.imgSrc) {
      console.log("designState", activeTemplate?.designState)
      setAppProps({ ...appProps, source: activeTemplate.designState.imgSrc, loadableDesignState: activeTemplate?.designState })
      if (showEditor) {
        setShowEditor(false);
  ...
  const onChangeBg = (bgImage: any) => {
    if (bgImage) {
      setAppProps({ ...appProps, source: bgImage });
    }
  }
  ...
  return (
    <React.StrictMode>
      {showEditor && <>
        {/* <ThemeProvider theme={defaultAndProvidedConfig.theme}>
          <FontsFaces />
          <OverrideDefaultStyles />
          <AppProvider config={defaultAndProvidedConfig}> */}
        <>
                          ...
                          <ThemeProvider theme={defaultAndProvidedConfig.theme}>
                            ...
                            <AppProvider config={defaultAndProvidedConfig}>
                              <Backgrounds
                                ...
                                appProps={appProps}
                                onChangeBg={(newbg) => onChangeBg(newbg)}
                                onBgChangeActionSelect={(from) => onBgChangeActionSelect(from)}
                              />
                              <Templates
                                editorConfig={editorConfig}
                                resparkTemplates={resparkTemplates}
                                setResparkTemplates={(templates) => setResparkTemplates(templates)}
                                templatesList={templatesList}
                                setTemplatesList={(templates) => setTemplatesList(templates)}
                                onSelectTemplate={(template) => onSelectTemplate(template)}
                                onDeleteTemplateClick={(event: any, template: any, templateIndex: number) => onDeleteTemplateClick(event, template, templateIndex)}
                              />
                              <App />
                            </AppProvider>
                          </ThemeProvider>
